...
class FirebaseChatCore {
  ...
  Future<void> createUserInFirestore(types.User user) async {
    await getFirebaseFirestore().collection(config.usersCollectionName).doc(user.id).set({
      ...
      'normalise_phone_number': user.normalise_phone_number,
  ...
  Stream<List<types.Message>> messages(
    ...
    var query =
        getFirebaseFirestore().collection('${config.roomsCollectionName}/${room.id}/messages').orderBy('createdAt', descending: true);
